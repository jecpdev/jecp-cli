...
// This scaffold uses JecpProvider.createHandler, which buffers the full
// response and returns it as a single JSON envelope. If you declare an
// action with \`streaming: true\` in jecp.yaml, the Hub will forward a
// streaming request and EXPECT a Server-Sent Events response back; this
// handler will return a non-SSE envelope and clients will see broken
// stream parsing.
//
// To ship a streaming action, replace this scaffold's handler with one
// that returns an SSE Response: see https://jecp.dev/guides/streaming
// for the Provider-side SSE wire format and reference handler.
// ─────────────────────────────────────────────────────────────────────────
//
...
\`streaming: true\` on any action in \`jecp.yaml\`, the Hub will forward
a streaming request and **expect an SSE response back** — this scaffold
will return a non-SSE envelope and clients will see broken stream parsing.

**To ship a streaming action**:

1. Keep this scaffold for the non-streaming actions you have.
2. For each \`streaming: true\` action, replace its handler with one that
   returns a fetch-API \`Response\` whose body is an SSE-encoded stream.
   See <https://jecp.dev/guides/streaming> for the wire format
   (\`Content-Type: text/event-stream\`, \`data: <json>\\n\\n\` framing,
   final \`event: done\` marker).
3. Smoke-test with \`jecp invoke <namespace>/<capability> <action> --stream\`
   from the CLI before publishing.

If you only need non-streaming actions for now, leave \`streaming: false\`
in every action and this scaffold works as-is.
...
- Streaming wire format: https://jecp.dev/guides/streaming
`;
}
