 ...
 *
 * Why hand-rolled rather than ajv: ajv adds ~250KB to the CLI bundle and we
 ...
 * or hit the Hub. This is intentional: a Hub error you've seen before is a
 * Hub error you can fix.
 */

import schema from '../schemas/manifest.schema.json' assert { type: 'json' };

export interface ValidationError {
  /** JSON Pointer into the instance (`/actions/0/pricing/base`). */
  instance_path: string;
  /** JSON Pointer into the schema (`/$defs/Pricing/properties/base/pattern`). */
  schema_path: string;
